...
typedef void(^ZZFileManagerStoreCompletedBlock)(NSString * _Nullable filePath);

typedef void(^ZZFileManagerQueryCompletedBlock)(NSData * _Nullable fileData);
...
typedef void(^ZZFileManagerNoParamsBlock)(void);
...
/**
 *  异步移除过期文件，无block回调。注意：如果设置了文件缓存最大空间值，如果移除后，缓存文件所占空间仍然大于阈值，那么会按照文件修改时间先后，先移除老文件，直到缓存文件所占空间小于等于阈值
 */
- (void)cleanDisk;
/**
 ...
 */
- (void)cleanDiskWithCompletionBlock:(nullable ZZFileManagerRemoveFileCompletionBlock)completionBlock;
/**
 *  获取当前磁盘缓存所占空间大小
 *
 *  @return 当前磁盘缓存所占空间大小
